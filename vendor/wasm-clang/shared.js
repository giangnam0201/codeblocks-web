...
class MemFS {
  ...
  host_read(fd, iovs, iovs_len, nread) {
    this.hostMem_.check();
    assert(fd === 0);
    // cbweb: when the program asks for input we do not have yet, hand control
    // back to the host so the console window can prompt the user; the host
    // throws, we unwind, and the program is re-run with the longer input.
    if (this.stdinStrPos >= this.stdinStr.length &&
        typeof this.onStdinStarved === 'function') {
      this.onStdinStarved();
    }
    let size = 0;
    for (let i = 0; i < iovs_len; ++i) {
...
class App {
  ...
  async run() {
    ...
    try {
      ...
      // cbweb: our own control-flow signals unwind silently.
      if (exn && exn.cbwebQuiet) throw exn;
      let writeStack = true;
